var LAYOUT = {
		navigations: function() {
			var navCategory = {
				information:	['intro', 'color', 'outline', 'grid', 'module'],
				convention:		['PSD'],
...
var FRONTUI = {
	markupList: function() {
		...
		$('#markupList tbody tr').each(function(i) {
			...
			if($(this).find('td').eq(-1).children().length < 1) {
...
//				var validation = $('<a href="http://validator.w3.org/check?uri=' + markupServer + '/html/' + markupDirectory + '/' + markupScreenID + '.html" target="_blank" class="bui-btn theme-a x-small ' + markupStateClass + '"><span class="text">' + markupState + '</span></a>');
	...
	markupProgressCheck: function() {
		...
		var countTotal = $('<span class="count-total">전체 ' + $('#markupList tbody tr').length + '</span>');
		var countUndefined = $('<span class="count-undefined">미정 ' + $('td[title="미정"]').length + '</span>');
		var countInprogress = $('<span class="count-inprogress">진행 ' + $('td[title="진행"]').length + '</span>');
		var countDone = $('<span class="count-complete">완료 ' + ($('td[title="완료"]').length + $('td[title="변경"]').length) + '</span>');
		...
		var btnOpenAll = $("<button type='button' class='bui-btn theme-a small state-b'><span class='text'>전체 페이지 열기</span></button>");
